			...
			if (!ticking) {
				window.requestAnimationFrame(() => {
					if (navPlaceholder) {
						...
						// Switch to fixed when the placeholder (where nav naturally sits)
						// scrolls past the bottom of the viewport
						isFixed = rect.bottom < window.innerHeight - 60;
					}
					ticking = false;
				});
				ticking = true;
...
<!-- Placeholder to maintain space in document flow and hold nav position -->
<div bind:this={navPlaceholder} class="flex justify-center">
	<!-- Navigation - always in relative position when not fixed -->
	{#if !isFixed}
		<nav
			bind:this={navElement}
			class="z-50 flex items-center text-sm font-medium mx-auto w-fit"
		>
			<ul
...
<!-- Fixed navigation at bottom - slides in/out from bottom -->
<nav
	class="fixed left-1/2 -translate-x-1/2 z-50 flex items-center text-sm font-medium transition-all duration-500 ease-out {isFixed
