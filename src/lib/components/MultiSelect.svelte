    ...
    <div class="tokens overflow-scroll" class:showOptions on:click={handleTokenClick}>
      {#each Object.values(selected).splice(1) as s}
        <div class="token bg-gradient-green font-semibold uppercase tracking-wide pl-2 text-white text-sm rounded-full mr-4" data-id="{s.value}">
          ...
          {#if !readonly}
            <div class="token-remove rounded-full h-6 w-6 grid ml-1 cursor-pointer bg-green-600 hover:bg-green-700" title="Remove {s.name}">
              <svg class="icon-clear m-auto" xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24">
      ...
      <div class="actions">
        {#if !readonly}
          <input id={id} class="bg-transparent" autocomplete="off" bind:value={inputValue} bind:this={input} on:keyup={handleKeyup} on:blur={handleBlur} placeholder={placeholder} aria-label="Select {id}"/>
          <div class="remove-all bg-green-600 hover:bg-green-700" title="Remove All" class:hidden={!Object.keys(selected).length}>
