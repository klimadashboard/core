  ...
  <div
      ...
      >
          ...
          <g transform={`translate(0, ${innerChartHeight - yScale(tick)})`} class="text-gray-500">
            ...
            <text class="text-xs fill-current bg-white" x="2" y="-3">{prettifyTick(tick, isUppermost)} <tspan dx=2>{isUppermost ? unit : ""}</tspan></text>
          </g>
          {/each}
          </g>
          ...
          <g>
          <linearGradient id="grad-{category.key}-{category.color}" x1="0%" y1="0%" x2="0%" y2="100%">
          ...
          <rect
              class="bar" 
              width={visualisation == "grouped" ? barWidth / datapoint.categories.length || 0 : barWidth} 
              height={yScale(category.value) || 0} 
              fill={category.estimate ? "transparent" : category.color ? "url(#grad-" + category.key + "-" + category.color + ")" : "currentColor"}
              stroke={category.estimate ? "currentColor" : category.color ? category.color : "currentColor"}
          ...
          <g transform="translate({-barWidth / 2},{innerChartHeight * 0.15})" class="text-gray-700 dark:text-gray-300">
              ...
              <svg xmlns="http://www.w3.org/2000/svg" class="icon icon-tabler icon-tabler-corner-right-down-double" width="24" height="24" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" fill="none" stroke-linecap="round" stroke-linejoin="round">
                  ...
                  <path d="M5 4h6a3 3 0 0 1 3 3v7"></path>
                  <path d="M10 10l4 4l4 -4m-8 5l4 4l4 -4"></path>
               </svg>
          </g>
          {/if}
          ...
          <line x1={margin.left} y1={innerChartHeight + margin.top || 0} x2={chartWidth || 0} y2={innerChartHeight + margin.top || 0} stroke="currentColor" stroke-width="2"></line>
          </g>
      </svg>
      {/if}
      ...
      <div class="absolute top-0 left-4 flex flex-wrap gap-2 px-4 md:px-0">
          {#if selectedBar.categories}
              {#each selectedBar.categories.filter(c => show0ValuesInLegend ? c.value > -1 : c.value > 0) as category}
                  <div class="font-semibold tracking-wide px-3 py-1 text-white text-xs rounded-full" style="background-color:{category.color || "#41AB5D"}" transition:fade="{{ duration: 300 }}">
