<script>
	...
	const getColor = function (index) {
		const colorScale = scaleLinear().range(colors).domain([-5, 0, 5, 7.5, 10, 12.5]);
					...
					{#each months as month, i}
						<g
							transform="translate({(Math.floor(
								(new Date(2000, month, 1) - new Date(2000, 0, 1)) / (1000 * 60 * 60 * 24)
							) *
								innerChartWidth) /
								365}, 0)"
						...
						>
							{#if i % 2 == 0}
								<rect height={innerChartHeight} width={innerChartWidth / 12} class="fill-gray-50" />
							{/if}
							<text
								fill="currentColor"
								x={innerChartWidth / 24}
								y={5}
								text-anchor="middle"
								dominant-baseline="hanging"
								>{new Date(2000, month, 1).toLocaleString('de-AT', { month: 'short' })}</text
							>
						</g>
					{/each}
				</g>
				...
				<g transform="">
					{#each yScale.ticks(5).splice(0) as tick}
						<g transform={`translate(0, ${yScale(tick)})`} class="text-xs text-gray-500">
							<text
								fill="currentColor"
								x={innerChartWidth - 22}
								dominant-baseline="middle"
								text-anchor="end">{tick}°C</text
							>
							<line
								class="text-white"
								x1="0"
								...
								<g transform="translate({xScale(day.day)},{yScale(day.t)})">
									...
									{#if day.heatDay}
										<circle
											r={circleRadius}
											fill={types[0].color}
											class="{commonClasses} {selectedDay && selectedDay !== index
												? 'opacity-50'
												: 'opacity-100'}"
										/>
					...
					{#if selectedDay}
						...
						>
							<path
								...
								class="text-gray-100 fill-current shadow stroke-gray-200"
							/>
							<rect />
							<text
