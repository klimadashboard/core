		...
		domtoimage
			...
			.then(async function (blob) {
				...
				try {
					...
					a.href = url;
					a.download = `Temperatur-seit-${year}.png`;
					a.click();
				}
			});
	...
	<div class="container flex flex-col items-center">
		<div class="flex flex-col md:flex-row items-center gap-8">
					...
					<div bind:this={item} class="h-full">
						<svg width={'100%'} height={'100%'} class="text-gray-100">
							...
							<g class="text-4xl text-gray-600" transform="translate(15,50)">
								...
								<text class="shareable-heading" y="40"
									>sich {PUBLIC_VERSION == 'at' ? 'Österreich' : 'Deutschland'}</text
								>
							...
							<g transform="translate(5,180)">
								{#each relevantDataset as datapoint, i}
									<g
										transform={`translate(${xScale(datapoint.year)}, ${yScale(
			...
			<div class="text-white w-80">
				<h3 class="text-2xl md:text-3xl leading-tight">
					Erstelle deine persönliche Temperaturgeschichte.
				</h3>
				<p class="text-lg mt-4 leading-snug">
					Gib dein Geburtsjahr ein und finde heraus, wie sehr sich {PUBLIC_VERSION == 'at'
						? 'Österreich'
						: 'Deutschland'} seitdem im Flächenmittel erhitzt hat.
				</p>
