<script>
    ...
    const regions = ["Austria", "Burgenland", "Kärnten", "Niederösterreich", "Oberösterreich", "Salzburg", "Steiermark", "Tirol", "Wien", "Vorarlberg"];

    $: selectedRegion = "Austria";

    $: dataset = rawData?.filter(d => d.region == selectedRegion).map((entry, i) => {
        return {
            x: i,
            label: entry.year,
            gas: entry.gas_sum,
