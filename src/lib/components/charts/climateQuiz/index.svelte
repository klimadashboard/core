<script lang="ts">
	...
	async function postAnswerToDb() {
		try {
			const postAnswer: DbQuizAnswer = {questionUuid: quizQuestions[questionIndex].uuid, answerId: clicked} 
	...
	async function getOtherAnswersFromDb(): Promise<DbCountQuizAnswer[]> {
		try {
				...
				<div class="max-w-prose">
					{#each quizQuestions[questionIndex].answers as answer, i}
						...
						<div class="quiz-button">
							<button
								class="relative bg-white text-black p-2 m-1 block w-full rounded-md {
									isCorrect 
