<script>
    ...
    $: selectedSectors = activeView == "sector_overview" ? sectors : views.filter((e) => e.key == activeView);
      ...
      if(entry.region == selectedRegion) {
            result.push({
                ...
                categories: selectedSectors?.map(function(item, index) {
                    return {
                      label: item.label,
                      value: showPerCapita ? Math.round(entry[item.key + perCapitaString] * 1000) / 1000 : Math.round(entry[item.key]),
                      color: item.color
                    };
                })
            })
    ...
    $: allowFlightEmissions = selectedRegion == "Austria" &&  (activeView == "sector_overview" || activeView == "total_co2e_t") && !showPerCapita;
    ...
    $: if(showFlightEmissions == true && allowFlightEmissions) {
      dataset = dataset.map(d => {
        ...
        categories.push({
          label: "Flug",
          value: Math.round(rawDataAustria.findIndex(e => e.year == d.label) > -1 ? rawDataAustria.find(e => e.year == d.label).international_flight_co2e_t : 0),
          color: "#7586C1"
        });
        return {
        label: d.label,
    ...
    <select bind:value={selectedRegion} class="block appearance-none w-full bg-gray-200 border border-gray-100   py-3 px-4 pr-8 rounded leading-tight focus:outline-none focus:bg-white focus:border-gray-500 max-w-sm">
      ...
      {#if allowFlightEmissions}
        <label class="flex gap-1 text-sm items-center {showFlightEmissions ? "text-blue-700" : "text-gray-400"}" transition:fade>
          <svg xmlns="http://www.w3.org/2000/svg" class="w-5 h-5 icon icon-tabler icon-tabler-plane" width="24" height="24" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" fill="none" stroke-linecap="round" stroke-linejoin="round">
            <path stroke="none" d="M0 0h24v24H0z" fill="none"></path>
    ...
    {/if}
    <label class="flex gap-1 text-sm items-center {freezeYAxis ? "text-gray-700" : "text-gray-400"}" transition:fade>
      <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
        ...
        <path d="M17 14.4444H12C11.4477 14.4444 11 14.9419 11 15.5556V18.8889C11 19.5025 11.4477 20 12 20H17C17.5523 20 18 19.5025 18 18.8889V15.5556C18 14.9419 17.5523 14.4444 17 14.4444Z" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>
        <path d="M12.5 14.4444V12.2222C12.5 11.6329 12.7107 11.0676 13.0858 10.6509C13.4609 10.2341 13.9696 10 14.5 10C15.0304 10 15.5391 10.2341 15.9142 10.6509C16.2893 11.0676 16.5 11.6329 16.5 12.2222V14.4444" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>
      </svg>        
        <span>Y-Achse fixieren?</span>
      <input type=checkbox bind:checked={freezeYAxis}>
    </label>
    <label class="flex gap-1 text-sm items-center {showPerCapita ? "text-gray-700" : "text-gray-400"}" transition:fade>
      <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 icon icon-tabler icon-tabler-users" width="24" height="24" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" fill="none" stroke-linecap="round" stroke-linejoin="round">
        <path stroke="none" d="M0 0h24v24H0z" fill="none"></path>
