<script>
	...
	Papa.parse('https://data.klimadashboard.org/at/energy/renewables/ausbauziele.csv', {
		download: true,
		dynamicTyping: true,
		skipEmptyLines: true,
		header: true,
	...
	Papa.parse('https://data.klimadashboard.org/at/energy/renewables/stromproduktion.csv', {
		download: true,
		dynamicTyping: true,
		skipEmptyLines: true,
		header: true,
	...
	{#if electricityProduction && expansionGoals}
		{#each energyTypes as type}
			<Chart
				{type}
				{unifiedScaling}
				dataProduction={electricityProduction
					.filter((d) => d[type.key])
					.map((d) => {
						return {
							x: new Date(d.date),
