    ...
    >
        <g>
            ...
            <g transform={`translate(${xScale(tick)}, ${chartHeight})`} class="text-white">
              <rect x={0} y={-chartHeight} width={chartWidth / 15} height={chartHeight} class="fill-gray-200 opacity-50"></rect>
              <!--
              ...
              <line x1="{chartWidth / 12}" x2="{chartWidth / 12}" y1="{-chartHeight}" y2={0} stroke-width="1" class="stroke-current" />
              -->
              <text class="text-xs text-gray-800 fill-current" x="6" y="-8">{tick.getFullYear()}</text>
            </g>
            {/each}
        </g>
        <g>
          ...
          <g transform={`translate(0, ${yScale(tick)})`} class="text-gray-400">
            ...
            <text class="text-xs text-gray-800 fill-current bg-white" x="10" y="-8">{tick} {index == yScale.ticks(6).length - 1 ? " TWh" : ""}</text>
          </g>
          {/each}
      </g>
        ...
        <g transform="translate({xScale(dataProduction[dataProduction.length - 1].x)},{yScale(dataProduction[dataProduction.length - 1].y)})">
            <circle r=5 fill="{colors[1]}"></circle>
        ...
        <g transform="translate({xScale(dataGoals[dataGoals.length - 1].x)},{yScale(dataGoals[dataGoals.length - 1].y)})">
            <text style="color:{colors[0]}" class="text-xs font-semibold fill-current" text-anchor="end" x={-10} y={-2}>2030-Ziel: {Math.round(dataGoals[dataGoals.length - 1].y)} TWh Strom aus {type.label}</text>
