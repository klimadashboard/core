<script>
    ...
    $: if(dataset.length > 0) {
        ...
        lastYearEmissions = dataset[dataset.length - 1].energy + dataset[dataset.length - 1].industry + dataset[dataset.length - 1].waste + dataset[dataset.length - 1].agriculture + dataset[dataset.length - 1].other;
    }
    </script>
    
    <div class="h-80">
            <div bind:clientWidth={chartWidth}></div>
            {#if dataset.length > 0}
            <LineChart 
            showAreas={true}
            data={dataset}
