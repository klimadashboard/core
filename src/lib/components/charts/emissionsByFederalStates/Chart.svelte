...
<script>
    ...
    $: sectors = [
      {
        ...
        label: $_('federalStates.sectors.energy'),
        color: "energy",
      ...
      },
      {
        ...
        label: $_('federalStates.sectors.industry'),
        color: "industry",
      ...
      },
      {
        ...
        label: $_('federalStates.sectors.mobility'),
        color: "mobility",
      ...
      },
      {
        ...
        label: $_('federalStates.sectors.building'),
        color: "building",
      ...
      },
      {
        ...
        label: $_('federalStates.sectors.agriculture'),
        color: "agriculture",
      ...
      },
      {
        ...
        label: $_('federalStates.sectors.waste'),
        color: "waste",
      ...
      },
      {
        ...
        label: $_('federalStates.sectors.fluorinatedGased'),
    ...
    $: getColor = (feature) => {
      const dataForFeature = data.find((d) => d.region == feature.properties["name"] && d.year == selectedYear);
    ...
    $: sortStates = function(a,b) {
      ...
      if(sortKey == "population") {
        sortValue = data.find((d) => d.region == a && d.year == selectedYear).population - data.find((d) => d.region == b && d.year == selectedYear).population;
      } else {
        // sort by emissions
        sortValue = data.find((d) => d.region == a && d.year == selectedYear).total_co2e_t_percapita - data.find((d) => d.region == b && d.year == selectedYear).total_co2e_t_percapita;
      }
  ...
  <div class="">
    ...
    <div class="flex gap-2">
      <input type="range" min="1990" max="2019" bind:value={selectedYear} aria-label="{$_('helpers.selectYear')}">
  ...
  <div class="flex gap-2 mt-4 overflow-x-scroll">
    ...
    <p class="">{formatNumber(data.find(d => d.year == selectedYear && d.region == activeState)[sector.key.replace("_percapita", "")])} t gesamt</p>
    <p class="">{formatNumber(data.find(d => d.year == selectedYear && d.region == activeState)[sector.key])} t pro Kopf</p>
    </div>
    </div>
    {:else}
  ...
  <section class="xl:-mx-4 my-4 h-full" bind:clientHeight={chartHeight} bind:clientWidth={chartWidth}>
    ...
    >
    {$_('federalStates.emissionsPerCapita')}
    ...
    {sortAscending ? "→" : "←"}
    {/if}
    {$_('federalStates.population')}
    </text>
    </g>
   
    <g transform="translate(20, 20)">
    {#each selectedStates.sort(sortStates) as state, i}
    <g class="{activeState == state || activeState == "" ? "opacity-100" : "opacity-70"} transition" 
    transform="translate(0,{yScale(populations.slice(0,i).reduce((pv, cv) => pv + cv, 0)) || 0})" 
    transition:fade>
    <g>
      {#each selectedSectors as sector, k}
      {@const dimensions = {width: getLength(sector,state, false), height: (yScale(populations[i]) - padding)}}
      <g 
      ...
      <text fill="currentColor" dominant-baseline="hanging" text-anchor="end" x={dimensions.width - 10} y=10>{sectors.find(s => s.key == sector).label}</text>
      {/if}
      {#if activeState == state && dimensions.width > 120 && dimensions.height > 60}
      <g transform="translate(10,{yScale(populations[i]) - padding - 10})">
      <text y="-12" fill="currentColor">{formatNumber(data.find((d) => d.year == selectedYear && d.region == state)[sector.replace("_percapita","")])} t gesamt</text>
      <text fill="currentColor">{formatNumber(data.find((d) => d.year == selectedYear && d.region == state)[sector])} t pro Kopf</text>
      </g>
      {/if}
      </g>
      <rect 
    ...
    <text dominant-baseline="hanging">und {formatNumber(data.find(d => d.region == state && d.year == selectedYear).total_co2e_t_percapita)} t pro Kopf</text>
    </g>
    {/if}
    </g>
    {/if}
    ...
    {#if width && height && chartWidth && openMap}
      <svg {width} {height} x={chartWidth - width} preserveAspectRatio="xMidYMid meet" class="bg-white">
        <g>
