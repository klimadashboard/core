<script>
	...
	$: marks = [
		...
		},
		{
			...
			label:
				dataFuture.find((d) => d.year == 2040).year +
				" <tspan class='font-light'>" +
				v.markerFuture +
				'</tspan>'
		},
		{
			...
			label:
				dataProjection.find((d) => d.year == 2050).year +
				" <tspan class='font-light'>" +
				v.markerProjection +
				'</tspan>'
		}
	...
	<div class="container">
		...
		<div class="grid md:grid-cols-3 gap-8">
			<div class="col-span-2 relative w-full max-w-2xl h-60 sm:h-56">
				<div
					class="chart h-full w-full absolute inset-0"
					...
					{#if chartWidth && chartHeight}
						<svg width={'100%'} height={'100%'} class="text-white text-sm">
							<g transform="translate({margin.left},{margin.top})">
								...
								<path d={generateArea()(dataHistorical)} fill="url(#grad-area)" />
							...
							<g transform="translate({margin.left},{margin.top})">
								{#each marks as mark}
									<g
										transform="translate({xScale(mark.x)},{yScale(mark.y)})"
									...
									>
										...
										<g
											class="fill-current font-semibold text-xs {selectedMark == mark
												? ''
												: 'opacity-70'}"
										>
											{#if xScale(mark.x) < 100}
												<text y="-10" transform="" text-anchor="middle"
													>{mark.label}
													<!--
                            ...
                            -->
											...
											{:else if mark.x == 2040}
												<text text-anchor="start" x="10" y="-4"
													>{@html mark.label}
													<!--
                            <tspan dx="2" class="font-light">{mark.y} ppm</tspan>
                            -->
				...
				<div
					class="absolute"
					style="left: {xScale(dataHistorical[dataHistorical.length - 1].year) +
						margin.left * 2}px; bottom: {margin.bottom - 5}px"
				>
			...
			<div class="flex space-x-4 items-end pl-14 md:pl-0">
				<div
					class="w-6 h-full relative rounded-t-full border-b-2 text-xs"
					style="font-variant-numeric: tabular-nums; background: linear-gradient(180deg, rgba(255, 255, 255, 0) 0%, rgba(255, 255, 255, 0.3) 20%, rgba(255, 255, 255, 0.3) 100%)"
				>
