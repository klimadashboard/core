...
{:else}
<div class="p-4 border border-gray-100 rounded relative {chart.content.methods ? "pb-16" : ""}" id="{chart.content.identifier_string}" bind:this={item}>
  
  <div class="flex justify-between items-center">
        <h2 class="uppercase tracking-wide font-semibold text-sm">{chart.content.title}</h2>
        <div class="flex items-center gap-4 text-gray-700">
            <button on:mousedown={() => exportImage()} class="hover:text-black transition" aria-label="Als Bild exportieren">
            ...
            <button on:mousedown={() => copyEmbedCode()} class="hover:text-black transition" aria-label="Einbetten">
            ...
            <svg width="256" height="256" viewBox="0 0 256 256" fill="none" xmlns="http://www.w3.org/2000/svg" class="h-8 w-8 rounded"><rect width="256" height="256" fill="url(#paint0_linear_1_17)"></rect><path d="M119.45 88H53C50.7909 88 49 89.7909 49 92V164C49 166.209 50.7909 168 53 168H119.45C122.998 168 124.79 163.723 122.3 161.194L92.3872 130.806C90.8547 129.249 90.8547 126.751 92.3872 125.194L122.3 94.8061C124.79 92.2773 122.998 88 119.45 88Z" fill="#DBF0E0"></path><path opacity="0.6" d="M162.95 88H134.808C133.732 88 132.701 88.4337 131.948 89.203L96.7358 125.203C95.2152 126.758 95.2152 129.242 96.7358 130.797L131.948 166.797C132.701 167.566 133.732 168 134.808 168H162.95C166.498 168 168.29 163.723 165.8 161.194L135.887 130.806C134.355 129.249 134.355 126.751 135.887 125.194L165.8 94.8061C168.29 92.2773 166.498 88 162.95 88Z" fill="#DBF0E0"></path><path opacity="0.2" d="M197.95 88H178.808C177.732 88 176.701 88.4337 175.948 89.203L140.736 125.203C139.215 126.758 139.215 129.242 140.736 130.797L175.948 166.797C176.701 167.566 177.732 168 178.808 168H197.95C201.498 168 203.29 163.723 200.8 161.194L170.887 130.806C169.355 129.249 169.355 126.751 170.887 125.194L200.8 94.8061C203.29 92.2773 201.498 88 197.95 88Z" fill="#DBF0E0"></path><defs><linearGradient id="paint0_linear_1_17" x1="425" y1="8.00003" x2="16" y2="248" gradientUnits="userSpaceOnUse"><stop stop-color="#A3D58A"></stop><stop offset="1" stop-color="#28A889"></stop></linearGradient></defs></svg>
        </div>
    </div>
    ...
    <div id="tab-switcher" class="absolute rounded-b bottom-0 left-0 right-0 grid grid-cols-2 bg-gray-100 dark:bg-gray-900 text-sm md:text-base">
      <button on:mousedown={() => showNotices = !showNotices} class="{!showNotices ? "bg-white dark:bg-gray-800" : "border-t dark:border-t-gray-600"} py-2">
          Grafik
      </button>
      <button on:mousedown={() => showNotices = !showNotices} class="{showNotices ? "bg-white dark:bg-gray-800" : "border-t dark:border-t-gray-600"} py-2">
