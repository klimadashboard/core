	...
	<div class="container flex justify-between items-center">
		...
		<nav class="flex gap-4">
			...
			{#await promise then children}
				<ul
					class="bg-white gap-2 sm:gap-4 {showNav
					...
					{#each children.filter((d) => d.num > 0 && d.parent == 'klimadashboard-at') as child}
						<li class="group relative">
							...
							{#if children.filter((c) => child.id == c.parent && c !== child)}
								<ul
									class="sm:opacity-0 sm:pointer-events-none group-hover:pointer-events-auto group-hover:opacity-100 transition sm:absolute top-full -left-2 p-2 bg-white w-64"
								>
